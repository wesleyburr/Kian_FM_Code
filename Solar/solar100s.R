...
#tf <- fft(c(imp, rep(0, 2^ceiling(log2(10*nflt)))))
#tf <- tf[1:(length(tf)/2)]
#fr <- seq(0, 0.5/delT, length.out = length(tf))
#png("lp_tf.png", width = 800, height = 450)
#  plot(fr, Mod(tf)^2, type = 'l', log = 'y', xlab = 'Frequency', ylab = '', main = '')
#  abline(v = 0.5/sr, lty = 2, col = 2)
#dev.off()


#png("solarSpec_100s_hp.png", width = 800, height = 450)
#  plot(spec.mtm(hp, deltat = sr, dtUnits = 'second'), main = '')
#dev.off()

#sp <- spec.mtm(hp, deltat = sr, dtUnits = 'second', Ftest = TRUE, plot = FALSE)
#sp.dr <- dropFreqs(sp, 0.0011, nyquist)
#png("solarSpec_100s_hp_dr.png", width = 800, height = 450)
#  plot(sp.dr, main = '')
#dev.off()

#png("solarF_100s_hp_dr.png", width = 800, height = 450)
#  par(mfrow = c(2,1))
#  par(oma = c(4,4,0,4))
#  par(mar = c(0,0,1,1))
#  plot(sp.dr, Ftest = TRUE, main = '', xaxt = 'n')
#  abline(h = qf(c(1-1/N, 0.99999, 0.9999, 0.999), 2, 2*sp$mtm$k-2), lty = 2, col = 2)
#  plot(dropFreqs(sp.Solar, 0.0011, nyquist), Ftest = TRUE, main = '')
#  abline(h = qf(c(1-1/N, 0.99999, 0.9999, 0.999), 2, 2*K-2), lty = 2, col = 2)
#dev.off()
...
for(m in 1:M){
  ...
  mtext("Frequency (mHz)", side = 1, outer = TRUE, line = 2.2)
